...
namespace ProniaAB104.Areas.AdminPanel.Controllers
{
    [Area("AdminPanel")]
    ...
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> Create(CreateProductVM productVM)
        {
            ...
            if (!result)
            {
                ViewBag.Categories = await _context.Categories.ToListAsync();
                ViewBag.Tags = await _context.Tags.ToListAsync();
                ModelState.AddModelError("CategoryId", "CategoryId have not already");
                return View();
            }
            ...
            foreach (int tagId in productVM.TagIds)
            {
                ...
                if (!tagResult)
                {
                    ViewBag.Categories = await _context.Categories.ToListAsync();
                    ViewBag.Tags = await _context.Tags.ToListAsync();
                    ModelState.AddModelError("TagIds", "Wrong info Tag Id ");
                    return View();
                }
            }
            ...
            if (!productVM.MainPhoto.ValidateType("image/"))
            {
                ViewBag.Categories = await _context.Categories.ToListAsync();
                ViewBag.Tags = await _context.Tags.ToListAsync();
                ModelState.AddModelError("MainPhoto", "File dont type");
                return View();
            }
            if (!productVM.MainPhoto.ValidateSize(600))
            {
                ViewBag.Categories = await _context.Categories.ToListAsync();
                ViewBag.Tags = await _context.Tags.ToListAsync();
                ModelState.AddModelError("MainPhoto", "File dont size");
                return View();
            }
            ...
            if (!productVM.HoverPhoto.ValidateType("image/"))
            {
                ViewBag.Categories = await _context.Categories.ToListAsync();
                ViewBag.Tags = await _context.Tags.ToListAsync();
                ModelState.AddModelError("HoverPhoto", "File dont type");
                return View();
            }
            if (!productVM.HoverPhoto.ValidateSize(600))
            {
                ViewBag.Categories = await _context.Categories.ToListAsync();
                ViewBag.Tags = await _context.Tags.ToListAsync();
                ModelState.AddModelError("HoverPhoto", "File dont size");
                return View();
            }
            ...
            foreach (IFormFile photo in productVM.Photos)
            {
                ...
                {
                    TempData["Message"] += $"<p class=\"text-danger\">{photo.FileName} file tdont typel</p>";
                    continue;
                }
                if (!photo.ValidateSize(600))
                {
                    TempData["Message"] += $"<p class=\"text-danger\">{photo.FileName} file dont size</p>";
                    continue;
                }
        ...
        public async Task<IActionResult> Update(int id, UpdateProductVM productVM)
        {
            ...
            if (!result)
            {
                productVM.Categories = await _context.Categories.ToListAsync();
                productVM.Tags = await _context.Tags.ToListAsync();
                ModelState.AddModelError("CategoryId", "Dont category Id");
                return View(productVM);
            }
            if (productVM.MainPhoto is not null)
            {
                if (!productVM.MainPhoto.ValidateType("image/"))
                {
                    productVM.Categories = await _context.Categories.ToListAsync();
                    productVM.Tags = await _context.Tags.ToListAsync();
                    ModelState.AddModelError("MainPhoto", "File dont typel");
                    return View(productVM);
                }
                if (!productVM.MainPhoto.ValidateSize(600))
                {
            ...
            if (productVM.HoverPhoto is not null)
            {
                if (!productVM.HoverPhoto.ValidateType("image/"))
                {
                    productVM.Categories = await _context.Categories.ToListAsync();
                    productVM.Tags = await _context.Tags.ToListAsync();
                    ModelState.AddModelError("HoverPhoto", "File dont typel");
                    return View(productVM);
                }
                if (!productVM.HoverPhoto.ValidateSize(600))
                {
            ...
            foreach (int tagId in creatable)
            {
                ...
                if (!tagResult)
                {
                    productVM.Categories = await _context.Categories.ToListAsync();
                    productVM.Tags = await _context.Tags.ToListAsync();
                    ModelState.AddModelError("TagIds", "dont have type");
                    return View(productVM);
                }
            ...
            if (productVM.Photos is not null)
            {
                foreach (IFormFile photo in productVM.Photos)
                {
                    if (!photo.ValidateType("image/"))
                    {
                        TempData["Message"] += $"<p class=\"text-danger\">{photo.FileName} file dont type </p>";
                        continue;
                    }
                    if (!photo.ValidateSize(600))
                    {
